...
export default async function OverviewPanel() {
  ...
  return (
    <div>
      ...
      <h1 className="font-serif text-3xl text-zinc-900 dark:text-white mb-10">
        Vue d&apos;ensemble
      </h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {stats.map((stat) => (
          <Link
            key={stat.label}
            href={stat.href}
            className="group relative block rounded-lg border border-zinc-200 dark:border-white/10 bg-white dark:bg-white/[0.03] p-7 overflow-hidden hover:border-accent/40 shadow-sm dark:shadow-none transition-colors"
          >
            <div
            ...
            />
            <div className="font-serif italic text-5xl text-accent-strong dark:text-accent leading-none">
              {stat.value}
            </div>
            <div className="font-mono text-[10px] uppercase tracking-[0.2em] text-zinc-400 dark:text-white/40 mt-4">
              {stat.label}
            </div>
          </Link>
        ))}
